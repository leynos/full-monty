...
fn recording_start_with_observer(
    ...
    _external_functions: Vec<String>,
    ...
    let progress = run
        .start_with_observer(inputs, NoLimitTracker, PrintWriter::Stdout, observer.clone())
    ...
    A: FnOnce(Result<RunProgress<NoLimitTracker>, MontyException>),
{
    ...
    let RunProgress::FunctionCall(state) = fixture.progress else {
        panic!("expected function call progress");
    };

    world.call_id = Some(state.call_id);

    let result = state.resume(resume_value, PrintWriter::Stdout);
