...
fn extract_function_call<T: ResourceTracker>(progress: RunProgress<T>, context: &str) -> FunctionCallPayload<T> {
    ...
    let function_name = state.function_name.clone();
    let args = state.args.clone();
    let kwargs = state.kwargs.clone();
    let call_id = state.call_id;
    let method_call = state.method_call;
...
fn runtime_observer_emits_external_return_kinds(
    ...
    let progress = run
        .start_with_observer(vec![], NoLimitTracker, PrintWriter::Stdout, observer)
    ...
    match resume_case {
        ExternalResumeCase::Return => {
            let completion = call
                .resume(MontyObject::Int(7), PrintWriter::Stdout)
...
fn runtime_observer_tracks_os_call_requests(recording: (RuntimeObserverHandle, Arc<Mutex<Vec<RecordedEvent>>>)) {
    ...
    let completion = call
        .resume(MontyObject::Bool(false), PrintWriter::Stdout)
...
fn runtime_observer_tracks_method_call_requests(recording: (RuntimeObserverHandle, Arc<Mutex<Vec<RecordedEvent>>>)) {
    ...
    let completion = call
        .resume(MontyObject::Int(3), PrintWriter::Stdout)
...
fn runtime_observer_emits_control_and_operation_events_for_branching_code(
    ...
    let progress = run
        .start_with_observer(vec![MontyObject::Int(1)], NoLimitTracker, PrintWriter::Stdout, observer)
...
fn runtime_observer_repl_paths_emit_external_control_and_op_events(
    ...
    let progress = repl
        .start_with_observer("ext_fn(1)", PrintWriter::Stdout, observer.clone())
        .expect("repl start should pause at external call");
    let call = progress
        .into_function_call()
        .expect("expected REPL function-call progress");
    let call_id = call.call_id;
    let state = call.resume(MontyObject::Int(9), PrintWriter::Stdout);

    let progress = state.expect("repl resume should complete");
