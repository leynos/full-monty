...
fn executor_iter_resource_limit_on_resume() {
    ...
    let limits = ResourceLimits::new().max_allocations(5);
    let (name, args, _kwargs, _arg_runtime_ids, _kwarg_runtime_ids, _call_id, _, state) = run
...
fn executor_iter_resource_limit_multiple_function_calls() {
    ...
    let (name, args, _kwargs, _arg_runtime_ids, _kwarg_runtime_ids, _call_id, _, state) = state
...
fn assert_repr_timeout(code: &str, label: &str) {
    ...
    let limits = ResourceLimits::new();
    let (name, _args, _kwargs, _arg_runtime_ids, _kwarg_runtime_ids, _call_id, _, mut state) = run
        .start(vec![], LimitedTracker::new(limits), &mut PrintWriter::Stdout)
        .unwrap()
        .into_function_call()
