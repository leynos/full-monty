...
fn corrupted_run_progress_payload_fails_to_load(#[with("ext_fn([])")] started_progress: RunProgress<NoLimitTracker>) {
    ...
    let completion = resume_with_none(into_function_call(started_progress, "original call"));
    assert!(matches!(completion, RunProgress::Complete(_)));

    assert!(
        RunProgress::<NoLimitTracker>::load(&bytes).is_err(),
        "corrupted payload should not deserialize"
    );
}
